...
#include <vector>
#include <queue>
#include <unordered_map> // For fast key-value mapping (e.g., location name to floor or index)
#include <climits>       // For INT_MAX, used as infinity in Dijkstra's algorithm
#include <cctype>        // For character handling functions like tolower()
#include <algorithm>     // For general algorithms (e.g., std::min, std::max, etc.)
using namespace std;
...
// The Edge struct represents a connection from one node to another with a certain weight (distance).
struct Edge {
    int to;      // Index of the destination node
    int weight;  // Distance or cost to reach the destination node
};

// The Graph class manages the entire graph structure for the campus map.
class Graph {
    unordered_map<string, int> nameToIndex; // Maps location names (in lowercase) to unique indices for fast lookup
    vector<string> indexToName;             // Stores the original names of locations, accessible by index
    vector<vector<Edge>> adj;               // Adjacency list: for each node, stores a list of connected edges
    int nodeCount = 0;                      // Keeps track of the total number of unique nodes (locations)

public:
    // Adds a new node (location) to the graph if it doesn't exist yet.
    // Returns the index of the node (existing or newly added).
    int addNode(const string& name) {
        string lname = toLower(name);               // Convert name to lowercase for case-insensitive matching
        if (nameToIndex.count(lname))               // If the node already exists, return its index
            return nameToIndex[lname];
        nameToIndex[lname] = nodeCount++;           // Assign a new index to this location
        indexToName.push_back(name);                // Store the original name for display purposes
        adj.push_back({});                          // Add an empty adjacency list for this node
        return nodeCount - 1;                       // Return the new node's index
    }

    // Adds an undirected edge (connection) between two locations with a given distance.
    // If the locations do not exist, they are added to the graph.
    ...
    // Finds and displays the shortest path between two locations using Dijkstra's algorithm.
    // Allows filtering by stairs/elevator for vertical transitions.
    // Prints the path and total distance, or an error if no path is found.
    ...
    // Lists all locations in the graph, grouped and displayed by floor.
    // Each floor is printed in orange, and each location in light blue.
    void listLocations() {
